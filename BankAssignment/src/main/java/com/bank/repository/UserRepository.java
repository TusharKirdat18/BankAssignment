...
public interface UserRepository extends JpaRepository<User, Userprimarykey> {
	...
	/* public List<User> findByuserprimarykeyUserId(String userId); */
	
	
     /*jpql*/
	
	/*Query(value="select * from user_details where user_id=:userid and password=:password" nativeQuery=true)
	 * 
	 public String findByuserprimarykeyUserIdAndPassword(@Param(userid) String UserId,@Param(password) String password)
